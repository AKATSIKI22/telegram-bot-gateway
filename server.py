...
def submit_credit_application():
    ...
    db_exec("""
        ...
        (fullname, phone, inn, income, term, amount, payment, session_id, credit_history, user_chat_id, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        fullname, phone, inn, 0, term, amount, payment,
        session_id, credit_history, user_chat_id, "pending"
    ))
...
⚠️ Ссылка активна 24 часа
    ...
    # Отклонение заявки
        ...
        if row and row[0]:
            send_user(row[0], "❌ К сожалению, ваша заявка отклонена")
